        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                @isset($article->category)
                    <a href="{{ route('categories.show', $article->category) }}" class="btn btn-light mb-2">{{ $article->category->name }}</a>
                @endisset

                <h5 class="card-title">{{ $article->title }}</h5>
                <h6 class="card-subtitle text-muted">Posted {{ $article->created_at->diffForHumans() }} by <a href="{{ route('users.show', $article->user) }}">{{ $article->user->name }}</a></h6>
                <br>
